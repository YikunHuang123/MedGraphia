...
async def test_phase8_features():
    ...
    async with httpx.AsyncClient(base_url=BASE_URL, timeout=60.0) as client:
        ...
        resp = await client.post("/admin/pipeline/trigger", json=trigger_body, headers={"X-API-Key": ADMIN_KEY})
        ...
        for _ in range(5):
            ...
            resp = await client.get(f"/admin/pipeline/status?domain={domain}", headers={"X-API-Key": ADMIN_KEY})
            status_data = resp.json()
            print(f"   - Progress: {status_data.get('progress', 0):.1%}, Stage: {status_data.get('stage')}")
            if status_data.get("stage") in ["completed", "failed"]:
                break
        print(f"✅ Pipeline persistent tracking verified.")
        ...
        try:
            async with client.stream("POST", "/chat/stream", json=chat_payload, headers={"X-API-Key": ADMIN_KEY}) as response:
                ...
                if content_chunks > 0 and citations_received and done_received:
                    print(f"✅ Streaming protocol test passed! (Received {content_chunks} text chunks)")
                else:
                    print(f"❌ Streaming protocol test failed. Chunks: {content_chunks}, Citations: {citations_received}, Done: {done_received}")
        ...
        print("2. Check Langfuse Dashboard: Verify traces for 'chat_stream' and 'knowledge_subgraph'.")
