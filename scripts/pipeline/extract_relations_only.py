...
async def main():
    ...
    from medgraphia.domain import Chunk as ChunkObj
            ...
            for e in e_nodes:
                if e.get("cui") and e.get("label"):
                    ...
                    chunk_entities.append(
                        Entity(cui=e["cui"], label=e["label"], entity_type=EntityType(e_type_str))
                    )
    ...
    api_key = (
        ...
        if hasattr(cfg.openai_api_key, "get_secret_value")
        else str(cfg.openai_api_key)
    )
    ...
    print(
        f"Step 3: Processing {len(chunks)} chunks in batches of {BATCH_SIZE} (Concurrency: {CONCURRENCY})..."
    )
    ...
    for i in range(0, len(chunks), BATCH_SIZE):
        ...
        async with get_session() as session:
            await session.run(
                "MATCH (c:Chunk) WHERE c.chunk_id IN $ids SET c.re_processed = true", ids=chunk_ids
            )
