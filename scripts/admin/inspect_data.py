...
from medgraphia.domain import Language, SourceMeta, RawDocument
...
async def inspect_pipeline():
    ...
    try:
        async with PubMedConnector() as pubmed:
            pubmed_docs = await pubmed.fetch(PubMedFetchConfig(query="metformin randomized trial", max_results=1))
            if pubmed_docs:
                doc = pubmed_docs[0]
                print(f"Title: {doc.title}")
                print(f"Sections Count: {len(doc.sections)}")
                save_processed_result(doc, "pubmed_metformin")
    except Exception as e: print(f"PubMed Error: {e}")
    ...
    except Exception as e: print(f"FDA Error: {e}")

    # 3. EMA SmPC (PDF Parsing via Docling)
    print("\n" + "="*60)
    print("--- 🇪🇺 [3/4] EMA SmPC (PDF Structural Parsing) ---")
    try:
        local_pdf = Path("data/raw/ema_smpc/Icandra__previously_Vildagliptin___metformin_hydrochloride_Novartis_.pdf")
        if local_pdf.exists():
    ...
    except Exception as e: print(f"EMA/Docling Error: {e}")
    ...
    except Exception as e: print(f"DrugBank Error: {e}")
