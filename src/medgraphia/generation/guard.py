...
class LlamaGuard:
    ...
    def __init__(self) -> None:
        ...
        logger.info(
            "guardrails_initialized", model=self.model_name, provider=self.provider_enum.value
        )
        ...
        async with httpx.AsyncClient(timeout=600.0) as client:
            ...
            try:
                check_resp = await client.post(
                    f"{ollama_base}/api/show", json={"name": self.model_tag}
                )
            ...
            # Pull the model
            logger.info(
                "guardrails_model_missing_pulling",
    ...
    async def check_output(
        self, question: str, answer: str, history: list[Message] | None = None
    ) -> SafetyResult:
        """
    ...
    async def _call_llama_guard(self, role: str, conversation: str) -> SafetyResult:
        ...
        prompt = LLAMA_GUARD_SYSTEM_PROMPT.format(role=role, conversation_history=conversation)
        ...
        if "unsafe" in first_line:
            ...
            return SafetyResult(
                is_safe=False, category=category_str, reason=f"Violates {category_str}"
            )
