...
def render_message_with_citations(
    ...
    cite_map = {int(c.get("number", c.get("citation_number", 0))): c
                for c in citations
    ...
    def _replace(m: re.Match) -> str:
        ...
        if n not in cite_map:
            return m.group(0)
        return (
            f'<a href="#mg-cm-{message_key}-{n}" class="mg-cref">[{n}]</a>'
        )
    ...
    for n in sorted(cite_map):
        ...
        source = _safe(
            f"version: {c.get('source_version', 'n/a')}   "
            f"·   section: {c.get('section_path', '')}"
        )
