...
def _builder(mock_data: CommunitySummaryResult | None = None, min_size: int = 2) -> CommunityBuilder:
...
class TestNeo4jWrite:
    ...
    async def test_write_communities_neo4j_unavailable_no_raise(self):
        ...
        with patch("medgraphia.graph.queries.upsert_community", side_effect=ConnectionError("down")):
